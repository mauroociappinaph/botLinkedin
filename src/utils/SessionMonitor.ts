...
export class SessionMonitor {
     ...
     */
    public analyzeSessionPerformance(sessionStats: SessionStats): any {
        const errorRate = sessionStats.jobsProcessed > 0
            ? (sessionStats.errorsEncountered / sessionStats.jobsProcessed) * 100
            : 0;

        const successRate = sessionStats.jobsProcessed > 0
            ? (sessionStats.applicationsSubmitted / sessionStats.jobsProcessed) * 100
            : 0;

        const captchaRate = sessionStats.jobsProcessed > 0
        ...
        const analysis = {
            ...
            performance: {
                ...
                averageProcessingTime: sessionStats.averageProcessingTime,
                memoryUsageMB: Math.round(memoryUsageMB * 100) / 100,
            },
            health: {
                status: this.determineHealthStatus(errorRate, successRate, captchaRate, memoryUsageMB),
                recommendations: this.generateRecommendations(errorRate, successRate, captchaRate, memoryUsageMB),
            },
    ...
    public generateMonitoringReport(): any {
        ...
        const report = {
            timestamp: new Date(),
            currentMetrics: {
                memoryUsageMB: Math.round(currentMetrics.memoryUsage.heapUsed / (1024 * 1024) * 100) / 100,
                memoryLimitMB: this.alertConfig.maxMemoryMB,
    ...
    private collectPerformanceMetrics(): void {
        ...
        this.logger.debug('Performance metrics collected', {
            memoryMB: Math.round(metrics.memoryUsage.heapUsed / (1024 * 1024) * 100) / 100,
    ...
    private checkAlerts(): void {
        ...
        // Memory usage alert
        if (this.alertConfig.maxMemoryMB && memoryUsageMB > this.alertConfig.maxMemoryMB) {
            this.addAlert({
                type: 'memory',
                severity: memoryUsageMB > this.alertConfig.maxMemoryMB * 1.5 ? 'critical' : 'warning',
    ...
    private determineHealthStatus(
        ...
        if (this.alertConfig.maxErrorRate && errorRate > this.alertConfig.maxErrorRate) {
            issues.push('high_error_rate');
        }

        if (this.alertConfig.minSuccessRate && successRate < this.alertConfig.minSuccessRate) {
            issues.push('low_success_rate');
        }

        if (this.alertConfig.maxCaptchaRate && captchaRate > this.alertConfig.maxCaptchaRate) {
    ...
    private generateRecommendations(
        ...
        if (this.alertConfig.maxErrorRate && errorRate > this.alertConfig.maxErrorRate) {
            recommendations.push('Consider increasing delays between actions to reduce errors');
        }

        if (this.alertConfig.minSuccessRate && successRate < this.alertConfig.minSuccessRate) {
            recommendations.push('Review job search criteria to target more suitable positions');
        }

        if (this.alertConfig.maxCaptchaRate && captchaRate > this.alertConfig.maxCaptchaRate) {
            recommendations.push('Increase stealth measures and reduce automation speed');
        }
     ...
     */
    private analyzePerformanceTrends(history: PerformanceMetrics[]): any {
        if (history.length < 2) {
        ...
        return {
            ...
            timespan: `${Math.round((history[history.length - 1].timestamp.getTime() - history[0].timestamp.getTime()) / 1000)}s`,
        };
    }
