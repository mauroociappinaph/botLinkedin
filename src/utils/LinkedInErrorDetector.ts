...
export class LinkedInErrorDetector {
    ...
    private async detectRateLimit(): Promise<LinkedInBotError | null> {
        ...
        for (const selector of LinkedInErrorDetector.DETECTION_INDICATORS.RATE_LIMIT_SELECTORS) {
        ...
        for (const text of LinkedInErrorDetector.DETECTION_INDICATORS.RATE_LIMIT_TEXT) {
            if (pageText.includes(text)) {
                return new LinkedInBotError(
                    `LinkedIn rate limit detected: "${text}"`,
                    {
                        category: ErrorCategory.DETECTION,
                        severity: ErrorSeverity.HIGH,
                        url: this.page.url(),
        ...
        const response = this.page.mainFrame().childFrames().length > 0 ?
            null : await this.page.goto(this.page.url(), { waitUntil: 'domcontentloaded' });
    ...
    private async detectBotDetection(): Promise<LinkedInBotError | null> {
        ...
        for (const selector of LinkedInErrorDetector.DETECTION_INDICATORS.BOT_DETECTION_SELECTORS) {
            const element = await this.page.$(selector);
            if (element) {
                return new LinkedInBotError(
        ...
        for (const text of LinkedInErrorDetector.DETECTION_INDICATORS.BOT_DETECTION_TEXT) {
            if (pageText.includes(text)) {
                return new LinkedInBotError(
    ...
    private async detectAccountSuspension(): Promise<LinkedInBotError | null> {
        ...
        for (const selector of LinkedInErrorDetector.DETECTION_INDICATORS.SUSPENSION_SELECTORS) {
            const element = await this.page.$(selector);
            if (element) {
                return new LinkedInBotError(
        ...
        for (const text of LinkedInErrorDetector.DETECTION_INDICATORS.SUSPENSION_TEXT) {
            if (pageText.includes(text)) {
                return new LinkedInBotError(
    ...
    private async detectGeneralErrors(): Promise<LinkedInBotError | null> {
        ...
        for (const indicator of LinkedInErrorDetector.DETECTION_INDICATORS.ERROR_PAGE_INDICATORS) {
     ...
     */
    public async monitorForErrors(timeoutMs: number = 5000): Promise<LinkedInBotError | null> {
